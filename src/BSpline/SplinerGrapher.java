...
public class SplinerGrapher extends JFrame {
    private BSplineH b;
    ...
    private int width;
    private int height;
    public SplinerGrapher(BSplineH b, int width, int height){
        pos = b.evaluatePos(0);
...
//        pos.setXY(xS,yS);
        ...
        JPanel p = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g.setColor(new Color(49, 49, 114));
                for (int i = 0; i < x.length; i++) {
//                    g.drawOval(x[i],y[i],10,10);
                    g.fillOval(x[i],y[i],5,5);
                }

                double x = (int) (followerBasic.get(pos).getX() * width + width/2.0);
                double y = (int) (followerBasic.get(pos).getY() * height + height /2.0);
//                System.out.println("x:" + x);
//                System.out.println("y:" + y);
//                System.out.println("vel pre scale" + new Vector2D(x,y));
                pos.add(new Vector2D(x,y).setMagnitude(100));
