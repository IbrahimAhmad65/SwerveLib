 ...
 */
public class StandardCircleObstacle extends PathAugment implements Displayable {
     ...
     */
    public StandardCircleObstacle(int dimensions, DPoint position, double radius, double awayCoefficient, double awayPower) {
        super(dimensions);
...
//        return PathAugmentFunctions.GetEffect.getEffectStandardStream(vectorBetween, toTarget, orthVector, effect, awayCoefficient, awayPower, streamCoefficient, streamPower);
        DVector motion = PathAugmentFunctions.GetEffect.getEffectDirectSingleTermPolynomialDistanceFunction(vectorBetween, effect, awayCoefficient, awayPower, vectorBetween.getDimensions());
        motion.add(toTarget);
