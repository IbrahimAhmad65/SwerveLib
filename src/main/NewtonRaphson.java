...
public class NewtonRaphson {
    ...
    public double smartApproximate(double initial, int iterations, Function<Boolean, Double> threshold) {
        double current = initial;
        double oldCurrent = 0;
        for (int i = 0; i < iterations; i++) {
            oldCurrent = current;
    ...
    public static void main(String[] args) {
        NewtonRaphson newtonRaphson = new NewtonRaphson((x) -> {
            return (x * Math.pow(2.718, x)) - 5;
        }, (x) -> {
            return (x * Math.pow(2.718, x) + Math.pow(2.718, x));
        });
//        NewtonRaphson newtonRaphson = new NewtonRaphson((x)->{return(x -3);},(x)->{return 1;});
        double b = newtonRaphson.approximate(1, 6);
        System.out.println(b);
        b = newtonRaphson.smartApproximate(1, 6, (x) -> {
